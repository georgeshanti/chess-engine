...
func (b *Board) NormalizeOpponentPieces() {
	for i, piece := range b.Pieces {
		if piece == Present|Black|Pawn|HasMovedTwoSquares {
			b.Pieces[i] = Present | Black | Pawn | HasNotMovedTwoSquares
		}
	}
}
			...
			case Pawn:
				{
					...
					if file > 0 && GetPresence(b.Get(rank+direction, file-1)) == Present && GetColor(b.Get(rank+direction, file-1)) != White {
						newBoard := b.DuplicateForNextMove()
						newBoard.Set(7-rank, 7-file, Empty)
			...
			case Rook:
				{
					...
					for _, i := range moveLengthArray {
						if canMoveUpward && rank-i >= 0 {
							...
							if GetPresence(b.Get(rank-i, file)) == Empty || GetColor(b.Get(rank-i, file)) != White {
								newBoard := b.DuplicateForNextMove()
								newBoard.Set(7-rank, 7-file, Empty)
						...
						if canMoveLeft && file-i >= 0 {
							...
							if GetPresence(b.Get(rank, file-i)) == Empty || GetColor(b.Get(rank, file-i)) != White {
								newBoard := b.DuplicateForNextMove()
								newBoard.Set(7-rank, 7-file, Empty)
			...
			case Knight:
				{
					for _, offset := range []struct {
						...
						if rank+offset.i >= 0 && rank+offset.i <= 7 && file+offset.j >= 0 && file+offset.j <= 7 && (GetPresence(b.Get(rank+offset.i, file+offset.j)) == Empty || GetColor(b.Get(rank+offset.i, file+offset.j)) != White) {
							newBoard := b.DuplicateForNextMove()
							newBoard.Set(7-rank, 7-file, Empty)
			...
			case Bishop:
				{
					...
					for _, i := range moveLengthArray {
						if canMoveUpRight && rank-i >= 0 && file+i <= 7 {
							...
							if GetPresence(b.Get(rank-i, file+i)) == Empty || GetColor(b.Get(rank-i, file+i)) != White {
								newBoard := b.DuplicateForNextMove()
								newBoard.Set(7-rank, 7-file, Empty)
...
func (b *Board) IsOpponentInCheck() bool {
	...
	for _, i := range moveLengthArray {
		if canMoveUpRight && rank-i >= 0 && file+i <= 7 {
			if GetPresence(b.Get(rank-i, file+i)) == Present {
				canMoveUpRight = false
				if GetColor(b.Get(rank-i, file+i)) == White {
					if GetType(b.Get(rank-i, file+i)) == Bishop || GetType(b.Get(rank-i, file+i)) == Queen {
						return true
						// }
					} else if i == 1 && GetType(b.Get(rank-i, file+i)) == Pawn && White == Black {
						return true
					}
				}
			}
		...
		if canMoveUpward && rank-i >= 0 {
			if GetPresence(b.Get(rank-i, file)) == Present {
				canMoveUpward = false
				if GetColor(b.Get(rank-i, file)) == White && (GetType(b.Get(rank-i, file)) == Rook || GetType(b.Get(rank-i, file)) == Queen) {
					return true
				}
			}
		}
		...
		if canMoveLeft && file-i >= 0 {
			if GetPresence(b.Get(rank, file-i)) == Present {
				canMoveLeft = false
				if GetColor(b.Get(rank, file-i)) == White && (GetType(b.Get(rank, file-i)) == Rook || GetType(b.Get(rank, file-i)) == Queen) {
					return true
				}
			}
		}
	...
	for _, offset := range [][]int{
		...
		if rank+rankOffset >= 0 &&
			...
			(GetPresence(b.Get(rank+rankOffset, file+fileOffset)) == Present &&
				GetColor(b.Get(rank+rankOffset, file+fileOffset)) == White &&
...
func (b *Board) GetEvaluation() *BoardState {
	...
	if len(legalMoves) == 0 {
		flippedBoard := *b
		flippedBoard = flippedBoard.DuplicateForNextMove()
