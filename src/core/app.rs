...
pub struct App {
    ...
    pub current_board: RwLock<Board>,
    ...
    pub run_lock: RwLock<()>,
    pub thread_stats: ArrayBuilder<ThreadStat, 32>,
    pub positions_evaluated_acount: RwLock<usize>,
    pub frame_count: Mutex<usize>,
    pub input: RwLock<Input>,
    pub editing: RwLock<bool>,
    pub prompt: RwLock<FixedLengthString<64>>,
    pub start_time: std::time::Instant,
    pub status: RwLock<FixedLengthString<64>>,
...
impl App {
    ...
    pub fn new(computer_count: usize, queuer_count: usize) -> App {
        ...
        let mut app = App {
            ...
            thread_stats: ArrayBuilder::new(),
            computer_count: computer_count,
            queuer_count: queuer_count,
            positions_evaluated_acount: RwLock::new(0),
            frame_count: Mutex::new(0),
            input: RwLock::new(Input::new()),
            editing: RwLock::new(true),
            prompt: RwLock::new(prompt),
            start_time: std::time::Instant::now(),
            status: RwLock::new(FixedLengthString::new(b"Evaluating...")),
            current_depth: depth,
            waiter: waiter,
        };
...
pub fn run(app: Arc<App>) -> Result<(), Box<dyn std::error::Error>> {
    {
        ...
        let _unused = std::thread::Builder::new().name(format!("app_main")).spawn(move || {
            run_engine(app.clone(), app.thread_stats.len());
        }).unwrap();
    }
    {
        let app = app.clone();
    ...
    loop {
        ...
        if poll(Duration::from_millis(100))? {
            ...
            let mut editing = app.editing.write().unwrap();
            if *editing {
                log!("Editing");
                match event {
                    Event::Key(key_event) => {
                        ...
                        if key_event.code == KeyCode::Esc {
                            ...
                            *editing = false;
                            ...
                            drop(editing);
                            process_prompt(app.clone(), &prune_sender, &loop_prune_receiver, &reval_sender, &loop_reval_receiver);
                            let mut editing = app.editing.write().unwrap();
                            *editing = true;
                        } else {
                            log!("Forward to input");
                            app.input.write().unwrap().handle_event(&event);
                        }
                    },
                    _ => {},
                };
...
pub fn draw(app: Arc<App>, frame: &mut Frame) {
    let _unused = app.run_lock.write().unwrap();
    let thread_count = app.thread_stats.len();
    ...
    for i in 0..app.thread_stats.len() {
        draw_stat(frame, i, &app.thread_stats[i], status_pane[i]);
    }
    frame.render_widget(RawU16Buffer{buf: app.current_board.read().unwrap().d()}, board_pane.inner(Margin{m:1}));
    frame.render_widget(Block::default().borders(Borders::ALL), prompt_pane);
    frame.render_widget(*app.prompt.read().unwrap(), prompt_pane);
    frame.render_widget(FixedLengthString::<64>::new(app.input.read().unwrap().value.deref()), prompt_pane.inner(Margin{m:1}));
    ...
    let positions_len = {
        ...
        let lens = app.positions.len();
        for i in 0..app.positions.length {
    ...
    frame.render_widget(convert_usize_to_u8_string(*app.positions_evaluated_acount.read().unwrap()).alignment(Alignment::Right), positions_evaluated_pseudo_value_pane);
    let status = {
        let mut buf = FixedLengthString::<64>::new(&[b' '; 0]);
        buf.add_u8(&[b'T', b'i', b'm', b'e', b':', b' ']);
        buf.add(convert_usize_to_u8_string(app.start_time.elapsed().as_secs() as usize));
    ...
    let mut f = app.frame_count.lock().unwrap();
    *f += 1;
}
...
fn process_prompt(app: Arc<App>, prune_sender: &Sender<Board>, loop_prune_receiver: &Receiver<()>, reval_sender: &Sender<()>, loop_reval_receiver: &Receiver<()>) {
        ...
        let captures = match re.captures(input.value()){
            ...
            None => {
                *app.prompt.write().unwrap() = FixedLengthString::new(&[b'I', b'n', b'v', b'a', b'l', b'i', b'd', b' ', b's', b'y', b'n', b't', b'a', b'x', b'.', b' ', b'E', b'n', b't', b'e', b'r', b' ', b'm', b'o', b'v', b'e', b':']);
                input.reset();
                return;
            }
        };
        ...
        if get_presence(source_piece) == EMPTY || get_color(source_piece) == BLACK || !(get_presence(target_piece) == EMPTY || get_color(target_piece) == BLACK) {
            ...
            *app.prompt.write().unwrap() = FixedLengthString::new(&[b'I', b'n', b'v', b'a', b'l', b'i', b'd', b' ', b'm', b'o', b'v', b'e', b'.', b' ', b'E', b'n', b't', b'e', b'r', b' ', b'm', b'o', b'v', b'e', b':']);
            input.reset();
            return;
        }
        ...
        let next_board = {
            let current_board_state = app.positions.get(&current_board);
            if let Some(pointer_to_board) = current_board_state {
                ...
                match next_board {
                    ...
                    None => {
                        ...
                        *app.prompt.write().unwrap() = FixedLengthString::new(&[b'I', b'n', b'v', b'a', b'l', b'i', b'd', b' ', b'm', b'o', b'v', b'e', b' ', b'2', b'.', b' ', b'E', b'n', b't', b'e', b'r', b' ', b'm', b'o', b'v', b'e', b':']);
                        input.reset();
                        return;
                    }
                }
                ...
                *app.prompt.write().unwrap() = FixedLengthString::new(&[b'I', b'n', b'v', b'a', b'l', b'i', b'd', b' ', b'm', b'o', b'v', b'e', b' ', b'3', b'.', b' ', b'E', b'n', b't', b'e', b'r', b' ', b'm', b'o', b'v', b'e', b':']);
                input.reset();
                return;
            }
        };
        ...
        {
            let mut input = app.input.write().unwrap();
            let next_board_state = app.positions.get(&next_board);
            match next_board_state {
                Some(pointer_to_board) => {
                    ...
                    match *next_best_move {
                        None => {
                            ...
                            *app.prompt.write().unwrap() = FixedLengthString::new(&[b'C', b'a', b'n', b'n', b'o', b't', b' ', b'f', b'i', b'n', b'd', b' ', b'n', b'e', b'x', b't', b' ', b'b', b'e', b's', b't', b' ', b'm', b'o', b'v', b'e', b'.', b' ', b'E', b'n', b't', b'e', b'r', b' ', b'm', b'o', b'v', b'e', b':']);
                            input.reset();
                            return;
                        }
                        Some(next_best_move) => {
                            ...
                            *app.current_board.write().unwrap() = next_best_move.board;
                            input.reset();
                        }
                    }
                },
                None => {
                    ...
                    *app.prompt.write().unwrap() = FixedLengthString::new(&[b'H', b'a', b'v', b'e', b' ', b'n', b'o', b't', b' ', b'e', b'v', b'a', b'l', b'u', b'a', b't', b'e', b'd', b' ', b'p', b'o', b's', b'i', b't', b'i', b'o', b'n', b' ', b'y', b'e', b't', b'.', b' ', b'E', b'n', b't', b'e', b'r', b' ', b'm', b'o', b'v', b'e', b':']);
                    input.reset();
                    return;
                }
            }
